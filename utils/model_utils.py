...
def get_knn_feature(xyz, k=20):
    xyz = xyz.transpose(2, 1).contiguous()
    B, N, C = xyz.shape
    idx = knn_point1(k, xyz, xyz)
    grouped_xyz = index_points(xyz, idx) # [B, npoint, nsample, C]
    
    xyz_tile = xyz.view(B, N, 1, C).repeat(1, 1, k, 1)
    relative_xyz = grouped_xyz - xyz_tile
    relative_dis = torch.sqrt(torch.sum(torch.square(relative_xyz), axis=-1, keepdims=True))
    new_points = torch.cat([relative_dis, relative_xyz, xyz_tile, grouped_xyz], dim=-1).permute(0, 3, 2, 1).contiguous()

    return new_points
 
 
def get_knn_feature_mis(xyz, new_xyz, k=20):
    xyz = xyz.transpose(2, 1).contiguous()
    new_xyz = new_xyz.transpose(2, 1).contiguous()
    B, N, C = new_xyz.shape
    idx = knn_point1(k, xyz, new_xyz)
    grouped_xyz = index_points(xyz, idx) # [B, npoint, nsample, C]
    
    new_xyz_tile = new_xyz.view(B, N, 1, C).repeat(1, 1, k, 1)
    relative_xyz = grouped_xyz - new_xyz_tile
    relative_dis = torch.sqrt(torch.sum(torch.square(relative_xyz), axis=-1, keepdims=True))
    
    new_points = torch.cat([relative_dis, relative_xyz, new_xyz_tile, grouped_xyz], dim=-1).permute(0, 3, 2, 1).contiguous()
...
def knn_point1(nsample, xyz, new_xyz):
    """
    Input:
